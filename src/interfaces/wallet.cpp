...
public:
        ...
        const bool mlike) override
    {
        ...
        if (!m_wallet->CreateTransaction(*locked_chain, recipients, pending->m_tx, pending->m_key, fee, change_pos,
                fail_reason, coin_control, sign, songword, mlike)) {
            return {};
        }
    ...
    std::map<uint256, CWalletTx> listSongwordReceviedTransactions() override
    {
    ...
    std::map<uint256, CWalletTx> listSongwordSentTransactions() override
    {
        std::map<uint256, CWalletTx> transactions;
