...
const char * const BITCOIN_CONF_FILENAME = "songmoney.conf";
...
#else
    const char* pszModule = "songmoney";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Songmoney";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Songmoney";
#else
    // Unix
    return pathRet / ".songmoney";
#endif
#endif
}
